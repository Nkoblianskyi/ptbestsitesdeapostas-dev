...
export function PolicySection({ title, children }: PolicySectionProps) {
  return (
    <div className="mb-8">
      <h2 className="text-xl md:text-2xl font-bold mb-4 text-gray-900 border-b border-gray-200 pb-2">{title}</h2>
...
export function PolicyParagraph({ children }: PolicyParagraphProps) {
  return <p className="text-sm md:text-base leading-relaxed text-gray-900">{children}</p>
}
...
export function PolicyList({ items, type = "bullet" }: PolicyListProps) {
  ...
  return (
    <ListComponent className={`${listClass} pl-5 space-y-2 text-sm md:text-base text-gray-900`}>
...
export function PolicyLink({ href, children, target, rel }: PolicyLinkProps) {
  return (
    <a
      href={href}
      className="text-portugal-green hover:text-portugal-dark-green underline transition-colors"
      target={target}
      rel={rel}
    >
      {children}
